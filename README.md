...
23. Amazon for printing versions must be without ⬆️ and `**[⬆ Back to Top](#sample-link-to-first-question)**`. While generating `.pdf`, use `pandoc --from gfm+task_lists --to pdf README.md --output AB123_v1.2.3_PRINT.pdf --epub-cover-image=images/ebook.jpg --metadata title="⬆️ Abcda Befghi AB-123 (Abcda Befghi Something) Practice Tests Exams Questions & Answers" --metadata author="Daniel Danielecki" --toc --number-sections --shift-heading-level-by=-2` script. Additionally, replace `- [ ]` with `- ( )` and `- [x]` with `- (x)`.
