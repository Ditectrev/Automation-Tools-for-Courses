...
URL = 'https://vceguide.com/how-should-you-complete-the-code-109/'
MASTER_LIST = []
...
def parse_question(content, question_number_local, imagezz):
    ...
    question = content.find('p').text.partition('SHOW ANSWERS')[0]
    print(question)
    images = []
    ...
    try:
        ...
        question2 = re.search('(.+?)A. ', question) or question
        print(re.search('A. (.+?)B. ', question))
        ...
        pass
    outdf = pd.DataFrame({
        'question': '\n### ' + str(question2) if question2 else question + '\n',
...
def return_next_page(soup):
    next_url = None
    cur_page = soup.find('div', {'class': 'wrap_page'})
    search_next = cur_page.findNext('li', {'class': 'cur_chapter'})
...
def scrap_next_question(url):
    ...
    question = soup.findAll('div', {'class': 'entry-content'})
